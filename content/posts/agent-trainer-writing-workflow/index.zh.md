...
还没完，因为它非常了解这篇文章，配图也由它来做。它会生成提示词，如果你配了 Gemini 或 OpenAI 的 API，可以直接调用生图。但我属于比较追求「性价比」的那一挂，不想额外买 API，就让它直接去登录了 Gemini 的浏览器里，访问网站生成图片，下载后整理好再上传到博客。**当时看到它自己完成这个流程的时候我也很震惊**——希望大家多尝试给它一些复杂任务，不要预设做不成，试一下再说。
